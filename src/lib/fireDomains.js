...
class FireDomainService {
  constructor(contract, walletClient, publicClient, pinataConfig) {
    ...
    // Disable mock mode - use real Pinata service for production
    this.mockMode = false
    
    console.log('🔥 Fire Domain Service running with real Pinata IPFS integration')
  ...
  async isDomainAvailable(domainName) {
    try {
      ...
      if (error.message.includes('returned no data') || 
        ...
        throw new Error(`Contract not properly deployed at ${this.contract.address}. Please check your network connection and contract deployment.`)
      }
